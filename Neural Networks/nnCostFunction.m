...
for i=1:m
a1d=a1(i,:)';
a2(i,2:end)=Theta1*a1d;
a2(i,:)=[1 sigmoid(a2(i,2:end))];
a3=Theta2*a2(i,:)';
a3=sigmoid(a3);
h(i,:)=a3';
endfor;
ydup=repmat([1:num_labels],m,1);
for i=1:m
ydup(i,:)=(ydup(i,:)==y(i));
endfor;
for i=1:m
J+=(ydup(i,:)*log(h(i,:))')+((1-ydup(i,:))*(log(1-h(i,:))'));
endfor;
J=(-1/m)*J;
regfact=sum(sum(Theta1(:,2:end).^2))+sum(sum(Theta2(:,2:end).^2));
J+=(lambda/(2*m))*regfact;
...
delta3=h-ydup;
for i=1:m
delta2(i,:)=(Theta2'*delta3(i,:)').*(a2(i,:).*(1-a2(i,:)))';
endfor;
